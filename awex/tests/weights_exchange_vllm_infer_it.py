...
def main(args) -> None:
    ...
    try:
        ...
        if args.server_only:
            logger.info(
                "Inference servers are healthy; waiting for the training driver."
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--meta-server-addr", default="")
    parser.add_argument(
        "--comm-backend",
    ...
    parser.add_argument(
        "--server-only",
        action="store_true",
        help="Start vLLM engines and leave publication control to the training driver.",
    )
    args = parser.parse_args()
    if not args.server_only and not args.meta_server_addr:
        parser.error("--meta-server-addr is required unless --server-only is set")
